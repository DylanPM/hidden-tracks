...
export function GenreConstellationSelect({ onLaunch }) {
  ...
  // --- geometry (center and radii) ---
const CENTER_X = 400;
const CENTER_Y = 300;
const LAUNCH_R = 100;                 // inner green "Launch" circle radius

// Ticker text ring: pull it closer to the Launch circle
const TEXT_RING_R = LAUNCH_R + 18;    // was farther out; 18px gap reads tight but clean

// node sizes (keep your dynamic behavior but we need the values here)
const numCircles = children.length > 0 ? children.length : seeds.length;
const genreNodeR = children.length > 0
  ? Math.max(45, 85 - numCircles * 2)
  : 70;                                // unused when seeds showing
const seedNodeR  = seeds.length > 0
  ? Math.max(55, 95 - numCircles * 2)
  : 60;

// Orbits: make nodes tangent to the outer edge of the text ring
const childrenOrbitR = TEXT_RING_R + genreNodeR + 0.5; // +0.5 avoids sub-pixel overlap
const seedsOrbitR    = TEXT_RING_R + seedNodeR  + 0.5;
//     // Orbit geometry
//   const CENTER_X = 400;
//   const CENTER_Y = 300;
//   const LAUNCH_R = 90;
//   const TEXT_RING_R = LAUNCH_R + 18;           // center button
    ...
    <div className="min-h-screen bg-zinc-950 text-white relative">
      ...
      {/* Wider Back zone */}
        {navigationPath.length > 0 && (
          <button
            onClick={handleBack}
            style={{ width: `${CENTER_X - (childrenOrbitR + genreNodeR)}px` }}  // stop tangentially at outermost edge
          ...
          >
            <div className="flex flex-col items-center gap-3">
              <ChevronLeft size={80} className="text-green-400 group-hover:text-green-300 transition-colors animate-pulse" />
        ...
        <div className="relative w-full max-w-4xl">
          <svg width="100%" height="600" viewBox="0 0 800 600" className="overflow-visible">
            ...
            <g className={launchAnimation ? 'animate-bounce-subtle' : ''}>
             ...
             {/* animated ticker description (slower + larger) */}
...
{children.length > 0 && children.map((child, index) => {
  ...
  return (
    <g key={child.key} onClick={() => handleGenreClick(child.key)} className="cursor-pointer transition-all">
      <circle cx={cx} cy={cy} r={genreNodeR} fill="#18181b" stroke="#22c55e" strokeWidth="3"
              className="hover:fill-zinc-800 hover:stroke-green-400 transition-all" />
      {lines.map((ln, i) => (
        <text key={i} x={cx} y={cy - (lines.length - 1) * 7 + i * 14}
...
{seeds.length > 0 && children.length === 0 && seeds.map((track, index) => {
  ...
  const artistText = track.artist.length > 16 ? track.artist.slice(0, 16) + '…' : track.artist;
  const songText   = track.name.length   > 18 ? track.name.slice(0, 18)   + '…' : track.name;

  return (
    <g key={track.uri} onClick={() => handleTrackClick(track)} className="cursor-pointer transition-all">
      <circle cx={cx} cy={cy} r={seedNodeR}
              fill={isSelected ? "#22c55e" : "#18181b"}
              stroke="#22c55e" strokeWidth={isSelected ? "4" : "3"}
              className="hover:fill-zinc-800 transition-all" />
      <text x={cx} y={cy - 14} textAnchor="middle" dominantBaseline="middle"
            fill="white" fontSize="13" fontWeight="600" className="pointer-events-none">
        {artistText}
      </text>
      <text x={cx} y={cy + 10} textAnchor="middle" dominantBaseline="middle"
            fill="#a1a1aa" fontSize="11" className="pointer-events-none">
        {songText}
      </text>
    </g>
  );
