...
export function useFeatureMap(manifest, exaggeration = 1.2, activeFeatures = {}, siblingFeatures = null) {
  const positions = useMemo(() => {
    ...
    console.log('\n🎯 ROOT GENRE POSITIONS (after global normalization):');
    ...
    // DEBUG: Analyze overlaps and clustering
    console.log('\n🔍 OVERLAP ANALYSIS - Detecting collisions:');
    console.log('============================================');

    const overlaps = [];
    const keys = Object.keys(scaledResult);

    // Helper to determine relationship (reusing constants from above)
    const getRelationship = (key1, key2) => {
      const parts1 = key1.split('.');
      const parts2 = key2.split('.');
    ...
    for (let i = 0; i < keys.length; i++) {
      for (let j = i + 1; j < keys.length; j++) {
        ...
        const relationship = getRelationship(key1, key2);
        const isRoot1 = !key1.includes('.');
        const isRoot2 = !key2.includes('.');
        const bothRoot = isRoot1 && isRoot2;
        ...
        if (distance < minDistance) {
          overlaps.push({
            ...
            overlap: (minDistance - distance).toFixed(1),
            relationship,
            pos1: { x: node1.x.toFixed(1), y: node1.y.toFixed(1) },
            pos2: { x: node2.x.toFixed(1), y: node2.y.toFixed(1) }
          });
        }
      }
    }

    // Sort by severity (biggest overlap first)
